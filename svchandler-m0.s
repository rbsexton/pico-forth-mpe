...
.thumb_func    
.global isr_svcall  @  Make sure the linker puts it in the table.
...
@ is the stack.   For applications where the SVC handler is the highest 
@ priority handler, using the registers directly is an option.
@
@ Notes on register usage.
@ The Exception stacks r0-r3,r12,lr,pc,xPSR
@ This code needs two scratch variables - 
@ - The Address of the target function. 
@ - A pointer to the stack frame generated by SVC call.
@ It has to use that stack frame pointer after calling the function, 
@ so it has to be preserved via AAPCS rules.
@ So: two scratch registers that are not r0-r3
@
